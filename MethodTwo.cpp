...
public:
	...
	void recursive(int& , unsigned int, int&);
};
...
{											 //First, get CIGAR count. Then look for deletion,then matched pair then get the next soft clipped.
	...
	while (readPosition != CIGAR.length())
	{
		current = deci = temp =  0;
		int deca = 1;
		cigardigit.assign(5, -1);
		recursive(deci, readPosition, current);
		for(int i = 4; i > -1 ; i--)
		{
			if(cigardigit[i] != -1)
			{
				temp = cigardigit[i] * deca;
				current = current + temp;
				deca = deca * 10;
				readPosition++;
			}
		}
		if (CIGAR.substr(readPosition,1) == "D")
		{
			total = total - current;
		}
		if (CIGAR.substr(readPosition,1) == "M" )
		{
			ThreePrime  = true;
		}
		if (ThreePrime == true)
		{
			if (CIGAR.substr(readPosition,1) == "S")
			{
...
void InputSEQ::recursive(int &digit, unsigned int readposition, int& Current)
{
	string q;
	int t, s;
	if(	CIGAR.substr(readposition+1,1) == "0" || CIGAR.substr(readposition+1,1) == "1" || CIGAR.substr(readposition+1,1) == "2" ||
		...
		{
			digit++;
			recursive (digit, readposition+1, Current);
		}

	q = CIGAR.substr(readposition,1);
	istringstream iss(q); iss >> t;
	cigardigit[digit] = t;
	digit--;
}
