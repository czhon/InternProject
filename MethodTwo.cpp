...
void InputSEQ::inputData(string input, SoftclipInputted *&head1)		//Data input are more than 7 soft clipped and only 3' adapter,condition at extract.
{
...
void InputSEQ::extractSEQtarget(bool& value)
{
	string r , p;
...
Calculation::Calculation()
{
	...
	MERSlength = 6;
	FoundPosition = 0;
	Probability = HighestFrequency = TotalFrequency = AveragePosition  = StandardDeviation = 0.0;
...
void Calculation::initialfunction(SoftclipInputted *head1)
{
	...
	obtainprobability();
	//cout << "End of base extraction" << endl;
	deletelist();
}
...
bool Calculation::obtainprobability()
{
	...
	while (temp != NULL)
	{
		...
		n++;
		AveragePosition = AveragePosition + temp->Position;
		temp = temp->Next;
	}
	TotalFrequency = TotalFrequency + 4 ; 	// To increase the error probability to control quality
	HighestFrequency = temp1->Occurrence;
	HighestFrequency = HighestFrequency + 1;
	Probability = HighestFrequency/TotalFrequency;
	AveragePosition = AveragePosition/n;
	temp = temp1 = NULL;
	delete temp;
	delete temp1;
	if(Probability < 0.90)
...
{
	calculateSD();
	//cout << "The final result of MERS is : "  << WholeMERS << endl << endl;
	deletelist();
	SixMERS = WholeMERS;
}
...
void BackwardExtension(Calculation b, SoftclipInputted *head1, string &A, string &B , string &C)
{
	...
	for(int i =0 ; i < 99 ; i++ )
	{
		...
		if (StopExtend == true)
		{
			//cout << "The backward extension ended because probability goes too low." << endl << endl;
			break;
		}
		...
		if (StopExtend == true)
		{
			//cout << "The backward extension end" << endl;
			break;
		}
		b.outputResult();
		//cout << "End of " << i << " backward extension" << endl;
	}
}
...
void ForwardExtension(Calculation b, SoftclipInputted *head1, string &A, string &B, string &C)
{
	...
	for(int i =0 ; i < 99; i++ )
	{
		...
		if (StopExtend == true)
		{
			//cout << "The forward extension ended because probability goes too low." << endl << endl;
			break;
		}
		...
		if (StopExtend == true)
		{
			//cout << "The forward extension end" << endl;
			break;
		}
		b.outputResult();
		//cout << "End of " << i << " forward extension" << endl;
	}
	b.returnMERS(A,B,C);
}
...
//Number of found in next extent might get smaller because it reached the end of the sample.
//Because of duplication and length, 6 mers frequency might lower than whole mers search
